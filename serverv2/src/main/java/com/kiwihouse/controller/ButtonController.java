...
public class ButtonController extends BaseController{
	...
	@Autowired
	RoleResourceButtonService roleResourceButtonService;
	@Autowired
    ...
    @GetMapping("/info/{resourceId}/{roleId}")
    public Response buttonListById(@PathVariable Integer resourceId,@PathVariable Integer roleId,HttpServletRequest request){
        logger.info("告警转工单>> {}",new Log().setIp(request.getRemoteAddr()).setParam(resourceId).setParam(roleId));
        Response resultList = roleResourceButtonService.buttonListById(resourceId,roleId);
    ...
    public Map<String, Object> queryInfo(Integer page,Integer limit){
    	try {
    		map = buttonService.queryInfo(page,limit);
    		map.put("code", 0);
    ...
    public Response queryButtonsPower(){
        return buttonService.queryButtonsPower();
    }
